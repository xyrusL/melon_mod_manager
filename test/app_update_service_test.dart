  ...
  test('treats newer stable date release as an update', () async {
    final client = MockClient((request) async {
      if (request.url.path ==
        ...
        return http.Response(
          jsonEncode({
            'tag_name': 'v1.6.4-2026.03.30',
            'name': 'v1.6.4-2026.03.30',
            'html_url':
                'https://github.com/xyrusL/melon_mod_manager/releases/tag/v1.6.4-2026.03.30',
          }),
          200,
        );
      }
    ...
    final service = AppUpdateService(
      ...
      owner: 'xyrusL',
      repository: 'melon_mod_manager',
      currentVersionProvider: () async => '1.6.3-2026.03.30',
    );

    final result = await service.checkForUpdate();

    expect(result.hasUpdate, isTrue);
    expect(result.latestRelease.tagName, 'v1.6.4-2026.03.30');
  });
}
